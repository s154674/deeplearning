...
def this_also_trains(input_encoder):
  # Define environment
  ...
  torch.save(policy.state_dict, f'{argv[1]}_{argv[2]}checkpoint{t_stamp}.pt')
  print(f'Completed training at {t_stamp}')

def this_trained(input_encoder):
  # Define environment
  # check the utils.py file for info on arguments
  env = make_env(**make_env_kwargs)
  print('Observation space:', env.observation_space)
...
def this_trains_on_our_code(input_encoder):
  # Define environment
  # check the utils.py file for info on arguments
  env = make_env(**make_env_kwargs)
  print('Observation space:', env.observation_space)
...
if __name__ == '__main__' and argv[1].lower() in ['ppo', 'impala']:
  ...
  if int(argv[2]) == 1:
    print('thistrained')
    this_trained(encoder)
  elif int(argv[2]) == 2:
    print('thisalsotrains')
    this_also_trains(encoder)
  elif int(argv[2]) == 3:
    print('thistrainsonourcode')
    this_trains_on_our_code(encoder)
  else:
