...
def stabelized(input_encoder, name):
  # Define environment
  ...
  torch.save(policy.state_dict, f'starpilot_sta_{name}_checkpoint{t_stamp}.pt')
...
def non_stabelized(input_encoder, name):
  ...
  t_stamp = datetime.utcnow().replace(microsecond=0)
  torch.save(policy.state_dict, f'starpilot_nosta_{name}_checkpoint{t_stamp}.pt')
  print(f'Completed training at {t_stamp}')
...
if __name__ == '__main__' and argv[1].lower() in ['ppo', 'impala']:
  # Hyperparameters
  total_steps = 3e6 #maybe 25e6?
  num_envs = 32
  num_levels = 200
  num_steps = 256
  num_epochs = 3
  ...
  print(f'Training {argv[1].upper()} {argv[2]} with the following parameter dict:')
  dct = {}
  ...
  if int(argv[2]) == 1:
    ...
    encoder = get_encoder()
    make_env_kwargs['num_levels'] = 75
    non_stabelized(encoder, f"{argv[1].lower()}_re_0_{make_env_kwargs['num_levels']}")
    non_stabelized(encoder, f"{argv[1].lower()}_re_1_{make_env_kwargs['num_levels']}")
    non_stabelized(encoder, f"{argv[1].lower()}_re_2_{make_env_kwargs['num_levels']}")
    encoder = get_encoder()
    make_env_kwargs['num_levels'] = 200
    stabelized(encoder, f"{argv[1].lower()}_re_0_{make_env_kwargs['num_levels']}")
    stabelized(encoder, f"{argv[1].lower()}_re_1_{make_env_kwargs['num_levels']}")
    stabelized(encoder, f"{argv[1].lower()}_re_2_{make_env_kwargs['num_levels']}")
    encoder = get_encoder()
    make_env_kwargs['num_levels'] = 500
    stabelized(encoder, f"{argv[1].lower()}_re_0_{make_env_kwargs['num_levels']}")
    stabelized(encoder, f"{argv[1].lower()}_re_1_{make_env_kwargs['num_levels']}")
    stabelized(encoder, f"{argv[1].lower()}_re_2_{make_env_kwargs['num_levels']}")
  elif int(argv[2]) == 2:
    print(f'stabilized {argv[1].lower()} with resets')
    encoder = get_encoder()
    make_env_kwargs['num_levels'] = 75
    stabelized(encoder, f"{argv[1].lower()}_re_0_{make_env_kwargs['num_levels']}")
    stabelized(encoder, f"{argv[1].lower()}_re_1_{make_env_kwargs['num_levels']}")
    ...
    raise Exception('Please include which way to train the network ')
  
  # this_also_trains(encoder) # <- This is not supposed to be here 
